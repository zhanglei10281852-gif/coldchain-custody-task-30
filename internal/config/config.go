...
func Load() (Config, error) {
	cfg := Config{
		...
		DatabasePath:     envOr("DATABASE_PATH", "./data/coldchain.db"),
		BusinessTimezone: envOr("BUSINESS_TIMEZONE", "Asia/Shanghai"),
		LogLevel:         envOr("LOG_LEVEL", "info"),
	}
	var err error
	if cfg.HandoffTTL, err = durationEnv("HANDOFF_TTL", 30*time.Minute); err != nil {
		return Config{}, err
	}
...
func (c Config) Validate() error {
	...
	if c.HandoffTTL < time.Minute || c.HandoffTTL > 24*time.Hour {
