...
func newHTTPFixture(t *testing.T) *httpFixture {
	...
	user := domain.User{ID: "usr_http", Email: "ops@example.test", DisplayName: "HTTP Ops", PasswordHash: string(hash), Role: domain.RoleOperations, Status: domain.UserActive, Version: 1, CreatedAt: now, UpdatedAt: now}
...
func TestProtectedEndpointRequiresBearerToken(t *testing.T) {
	f := newHTTPFixture(t)
	response := f.request(t, http.MethodPost, "/api/v1/studies", map[string]any{}, "")
...
func TestLoginReturnsTokenAndCreatesStudy(t *testing.T) {
	f := newHTTPFixture(t)
	...
	response = f.request(t, http.MethodPost, "/api/v1/studies", map[string]any{"code": "STUDY-HTTP", "name": "HTTP study", "minimum_celsius": 2, "maximum_celsius": 8, "max_transit_hours": 12, "review_hours": 4, "business_timezone": "Asia/Shanghai"}, token)
	if response.StatusCode != http.StatusCreated {
		t.Fatalf("create study status = %d", response.StatusCode)
	}
	created := readResponse(t, response)
	if created["code"] != "STUDY-HTTP" || created["status"] != string(domain.StudyDraft) {
		t.Fatalf("created study = %+v", created)
	}
}
...
func TestUnknownJSONFieldAndOversizedBodyAreRejected(t *testing.T) {
	...
	unknown := f.request(t, http.MethodPost, "/api/v1/studies", map[string]any{"code": "X", "unexpected": true}, token)
	...
	req, err := http.NewRequest(http.MethodPost, f.server.URL+"/api/v1/studies", strings.NewReader(`{"code":"`+large+`"}`))
	if err != nil {
		t.Fatal(err)
	}
	req.Header.Set("Content-Type", "application/json")
