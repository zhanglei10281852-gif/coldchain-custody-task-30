...
func (w *Worker) RunOnce(ctx context.Context) error {
	...
	return w.store.WithTx(ctx, func(tx repository.Tx) error {
		expired, err := tx.ExpireHandoffs(ctx, now, w.batch)
		if err != nil {
			return err
		}
		if len(expired) > 0 {
			w.logger.Info("expired custody handoffs", "count", len(expired))
		}
...
func (w *Worker) processJob(ctx context.Context, tx repository.Tx, job domain.OutboxJob) error {
	...
	switch job.Kind {
	case "shipment_planned", "excursion_review":
		var marker string
		if len(job.Payload) > 0 {
			if err := json.Unmarshal(job.Payload, &marker); err != nil && job.Kind == "excursion_review" {
				return fmt.Errorf("decode excursion job: %w", err)
			}
		}
		return nil
	default:
